...
$accordionId = $accordionId ?? ($gridId . '-notes-accordion');
$collapseId = $collapseId ?? ($gridId . '-notes');
$headingId = $accordionId . '-header';
$collapseSelector = '#' . $collapseId;
...
if ($showNotes) {
    ?>
    <div class="accordion mb-3" id="<?= Html::encode($accordionId) ?>">
        <div class="accordion-item">
            <h2 class="accordion-header" id="<?= Html::encode($headingId) ?>">
                <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="<?= Html::encode($collapseSelector) ?>" aria-expanded="false" aria-controls="<?= Html::encode($collapseId) ?>">
                    <i class="fas fa-info-circle me-2"></i> Important Notes
                </button>
            </h2>
            <div id="<?= Html::encode($collapseId) ?>" class="accordion-collapse collapse" aria-labelledby="<?= Html::encode($headingId) ?>" data-bs-parent="#<?= Html::encode($accordionId) ?>">
...
?>
<div class="allocation-grid-container" data-allocation-grid>
    <?php
    try {
            ...
            'exportConfig' => [
                GridView::PDF => [
                    ...
                    'config' => [
                        ...
                        'methods' => [
                            'SetHeader' => [
                                '<div style="text-align:left; font-size:10pt; font-weight:bold;">University of Nairobi</div><div style="text-align:center; font-size:12pt;">Course Allocations Report</div><div style="text-align:right; font-size:9pt; color:#555;">{PAGENO}/{nbpg}</div>'
                            ],
                            'SetFooter' => [
                                '<div style="text-align:left; font-size:8pt; color:#777;">Generated on ' . date('Y-m-d H:i:s') . '</div><div style="text-align:right; font-size:8pt; color:#777;">Powered by Lecturer Allocation System</div>'
                            ],
                        ],
                        'cssInline' => 'body { font-family: DejaVu Sans, sans-serif; font-size:9pt; } table { border-collapse: collapse; } th { background-color: #f0f0f0; font-weight: bold; font-size:9pt; } td { font-size:8.5pt; padding: 4px; }',
                    ],
                ],
            ],
            'panel' => [
                'type' => GridView::TYPE_DEFAULT,
                'heading' => '<i class="fas fa-book"></i> ' . Html::encode($panelHeading . ' - ' . $filter->purpose),
                'headingOptions' => [
