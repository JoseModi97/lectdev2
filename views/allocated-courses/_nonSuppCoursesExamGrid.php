    ...
    'buttons' => [
        ...
        'enter-exam-marks' => function($url, $model){
            if(!SmisHelper::hasMultipleExamComponents($model->MRKSHEET_ID)) {
                /**
                 * For courses with coursework, the exam assessment is created once when the first coursework is created.
                 * We require that before exam marks are entered for these type of courses, the exam assessment be
                 * available. We can't enter exam before coursework marks.
                 * We have other courses that have no coursework assessments. For these, we create their exam assessments
                 * if missing, once when trying to save marks. This approach is also used for supplementary exams.
                 */

                $createExamsBtn = Html::a('<i class="fas fa-plus"></i> marks',
                    Url::to([
                        '/marks/create-exam',
                ...
                /**
                 * Look for created exam for this course.
                 * It's only present if a course work had been created.
                 */
                $examAssessment = SmisHelper::getExamAssessment($model->MRKSHEET_ID);

                if(SmisHelper::requiresCoursework($model->MRKSHEET_ID)){
                    if($examAssessment){
                        return $createExamsBtn;
                    }else{
                        return Html::button('<i class="fas fa-ban"></i> exam not found', [
        ...
        'edit-exam-marks' => function($url, $model){
            if(!SmisHelper::hasMultipleExamComponents($model->MRKSHEET_ID)) {
                $editExamsBtn = Html::a('<i class="fas fa-edit"></i> marks',
                    Url::to([
                        '/marks/edit-all-exam',
                        'marksheetId' => $model->MRKSHEET_ID
                    ]),
        ...
        'delete-marks' =>  function($url, $model){
            if(!SmisHelper::hasMultipleExamComponents($model->MRKSHEET_ID)) {
                $deleteMarksBtn = Html::a('<i class="fas fa-trash"></i> marks',
                    Url::to([
