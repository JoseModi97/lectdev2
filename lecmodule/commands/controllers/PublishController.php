            ...
            foreach ($studentsToPublish as $studentToPublish) {
                ...
                if (intval($studentIsInMarksheet) === 0) {
                    ...
                    if (!$tempMarksheet->save()) {
                        $message = $regNumber . ' in ' . $marksheetId . '  failed to update publish and marks complete.';

                        if (!empty($tempMarksheet->getErrors())) {
                            $message = $regNumber . ' in ' . $marksheetId . ' failed to update publish and marks complete. ' . json_encode($tempMarksheet->getErrors());
                        }
