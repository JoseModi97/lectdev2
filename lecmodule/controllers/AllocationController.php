...
class AllocationController extends BaseController
{
    ...
    {
        try{
            if($filtersFor !== 'nonSuppCourses' && $filtersFor !== 'suppCourses' && $filtersFor !== 'serviceCourses'
    ...
    public function actionViewRequestRender(string $requestId)
    {
        try{
            ...
            $this->setFlash('danger', 'Failed to update course leader',$message);
            return Yii::$app->response->redirect(['allocation/give']);
        }
    }
    ...
    public function actionAllocateRequestLecturer()
    {
        ...
        try{
            ...
            if($courseType === 'departmental' && $internalLecturer === 'true'){
                if(empty($lecturers)){
                    $this->setFlash('danger', 'Assign Courses',
            ...
            if(($courseType === 'departmental' &&  $internalLecturer === 'true') || $statusName === 'APPROVED'){
                $this->setFlash('success', 'Assign Courses',
            ...
            if($courseType === 'departmental' &&  $externalLecturer === 'true'){
                if($internalLecturer === 'true'){
                    if($requestExists){
                        $this->addFlash('danger', 'Request Courses',
                            'Request for this course was not sent. Another request exists that is still pending.');
                    }else{
                        $this->addFlash('success', 'Request Courses',
                            'A lecturer for this course has been requested successfully.');
                    }
                }
                else{
                    if($requestExists){
                        $this->setFlash('danger', 'Request Courses',
                            'Request for this course was not sent. Another request exists that is still pending.');
                    }else{
                        $this->setFlash('success', 'Request Courses',
                            'A lecturer for this course has been requested successfully.');
                    }
                }
            }
    ...
    public function actionManageAllocatedLecturer(): Response
    {
        ...
        try{
            ...
            $this->setFlash('success', 'Manage course lecturers',
    ...
    public function actionRemoveAllocatedLecturer(): Response
    {
        ...
        try{
            ...
            $transaction->commit();
            $this->setFlash('success', 'Remove allocated lecturers',
            ...
            $this->setFlash('danger', 'Failed to remove lecturer',$message);
            return Yii::$app->response->redirect(['allocation/give']);
        }
    }
